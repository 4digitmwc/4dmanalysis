...
def get_table_players(dataframe: pd.DataFrame):
    players = dataframe['player_name'].unique()
    rounds = dataframe['round'].unique()
    beatmap_type = dataframe['beatmap_type'].unique()
    table_dict = {}
    for player in players:
        ...
        for round in rounds:
            for cat in beatmap_type:
                score_player_round = dataframe[(dataframe['player_name'] == player) & (dataframe['round'] == round) & (dataframe['beatmap_type'] == cat)][['beatmap_tag', 'score_logit']]
                for beatmap_tag, score_logit in score_player_round.values:
                    player_dict["_".join([round, cat, str(int(beatmap_tag))])] = score_logit
